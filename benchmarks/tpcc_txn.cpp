...
RC tpcc_txn_man::run_new_order(tpcc_query * query) {
	...
	//double d_tax;
	int64_t o_id;
	int64_t d_tax = *(double *) r_dist_local->get_value(D_TAX);
	...
	insert_row(_wl->i_order, orderPrimaryKey(w_id,d_id,o_id),r_order, wh_to_part(w_id),O_order); //(r_order, _wl->t_order);
	/*=======================================================+
	...
	insert_row(_wl->i_new_order, orderPrimaryKey(w_id,d_id,o_id),r_no, wh_to_part(w_id),O_new_order);
	for (UInt64 ol_number = 0; ol_number < ol_cnt; ol_number++) {
